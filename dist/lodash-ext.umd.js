(function (global, factory) {
	...
	(global = global || self, factory(global._ = {}));
	...
	function deepCamelizeKeys(object) {
	  var camelized = cloneDeep_1(object);
	  forOwn_1(object, function (value, key) {
	    ...
	    var camelizedKey = camelCase_1(key);
	...
	var wellKnownSymbol = function (name) {
	  return store$2[name] || (store$2[name] = nativeSymbol && Symbol$2[name] || (nativeSymbol ? Symbol$2 : uid)('Symbol.' + name));
	};
	...
	*/
	...
	function canonic(string) {
	  ...
	  var text = string + '';
	  return deburr_1(text.trim().replace(/\s{2,}/g, ' ')).toLowerCase();
	}
	...
	function blank(value) {
	  switch (_typeof(value)) {
	    ...
	    default:
	      ...
	      return isEmpty_1(value) && !isNumber_1(value) || _isNaN(value);
	  }
	}
	...
	function deleteBlanks(object) {
	  var result = cloneDeep_1(object);
	  forOwn_1(result, function (value, key) {
	    if (isPlainObject_1(value) || isArray_1(value)) {
	      result[key] = deleteBlanks(value);

	      if (isArray_1(result[key])) {
	        remove_1(result[key], blank);
	      }
	...
	if (objectToString$1 !== ObjectPrototype.toString) {
	  redefine(ObjectPrototype, 'toString', objectToString$1, {
	    unsafe: true
	  });
	}
	...
	function parse(source, type) {
	  ...
	  if (isArray_1(source)) {
	    return map_1(source, function (v) {
	      return parse(v, type, options);
	    });
	  }

	  if (isPlainObject_1(source)) {
	    return reduce_1(keys_1(source), function (parsedObj, key) {
	...
	parse.remove = function remove(customParserTypes) {
	  var types = castArray_1(customParserTypes);
	  var unknownTypes = difference_1(types, keys_1($customParsers));
	...
	var $parsers = {
	  ...
	  string: function string(value) {
	    ...
	    if (isArray_1(value)) return value.toString();
	    if (isPlainObject_1(value)) return JSON.stringify(value);
	...
	function pickParse(object) {
	  ...
	  var keys = keys_1(config);

	  var picked = pick_1(object, keys);
	  return reduce_1(keys, function (pickedAndParsed, key) {
	    var type = get_1(config, key);
	    var value = get_1(picked, key);

	    var parsed = parse(value, type, options);

	    set_1(pickedAndParsed, key, parsed);
	...
	function search(source, target) {
	  ...
	  return includes_1(_source, _target);
	}
	...
	function deepSnakeizeKeys(object) {
	  var snakeized = cloneDeep_1(object);
	  forOwn_1(object, function (value, key) {
	    // checks that a value is a plain object or an array - for recursive key conversion
	    // recursively update keys of any values that are also objects
	    if (isPlainObject_1(value) || isArray_1(value)) {
	    ...
	    var snakeizedKey = snakeCase_1(key);
